...
class Payrollservice {
  ...
  Future<List<mymodel.Data>> getpayroll({
    int? branchid,
    int? month,
    int? currencyID,
    int? year,
    ...
    try {
      ...
      if (respone.statusCode == 200) {
        final json = respone.data;
        final model = mymodel.PayrollModel.fromJson(json);
        return model.data ?? [];
      } else {
        throw Exception("Failed: ${respone.statusCode}");
      }
  ...
  Future<bool> createBulkPayroll(List<Map<String, dynamic>> payrolls) async {
    try {
      ...
      CustomSnackbar.error(title: "កំហុស", message: "កំហុសក្នុងការតភ្ជាប់: $e");
      return false;
    }
  }
      ...
      final payrolls = [
        {
          ...
          'penalty_leave_without_permission':
              PenaltyLeaveWithoutPermission ?? 0.0,
          ...
          'totalDeductions':
              TotalDeduction ?? 0.0, // Changed to totalDeductions
  ...
  Future<bool> deletepayroll({required int Id}) async {
    try {
      final response = await apiProvider.delete('deletepayroll/${Id}');
      if (response.statusCode == 200) {
        return true;
      } else {
        CustomSnackbar.error(
          title: "បរាជ័យ",
          message: "លុបមិនបានទេ: ${response.statusCode}",
        );
        return false;
      }
    } catch (e) {
