...
class Payrollcontroller extends GetxController {
  ...
  Future<void> fetchpayroll({
    ...
    try {
      ...
      final result = await payrollservice.getpayroll(
        branchid: branchid,
        month: month,
        currencyID: currencyID,
        year: year,
      );
      payroll.assignAll(result);
    } catch (e) {
  ...
  Future<void> submitAllPayrolls({
    ...
    try {
      ...
      for (var payrollData in payrolldraff) {
        try {
          ...
          double totalDeductions =
              (payrollData.totalDeductions ?? 0) + loanDeduction;
          ...
          Map<String, dynamic> payrollRequest = {
            ...
            'payrollmonth': month,
            'notdeduction':
                payrollData.baseSalary?.toInt() ?? 0, // Using notdeduction
            ...
            'penalty_leave_with_permission':
                payrollData.penaltyLeaveWithPermission ?? 0.0,
            'leave_without_permission': payrollData.leaveWithoutPermission ?? 0,
            'penalty_leave_without_permission':
                payrollData.penaltyLeaveWithoutPermission ?? 0.0,
            'leave_weekend': payrollData.leaveWeekend ?? 0,
            'penalty_leave_weekend':
                payrollData.penaltyLeaveWeekend?.toInt() ?? 0,
            ...
            'bonus_type': payrollData.isBonusAttendanace == 1
                ? "Attendance Bonus"
                : "",
            'bonus_amount': payrollData.isBonusAttendanace == 1
                ? (payrollData.bonusAmount?.toDouble() ?? 0.0)
                : 0.0,
      ...
      if (payrollRequests.isNotEmpty) {
        bool isCreated = await payrollservice.createBulkPayroll(
          payrollRequests,
        );
  ...
  Future<void> deletepayroll({required int ID})async{
    try{
    isLoading.value = true;
    final result = await payrollservice.deletepayroll(Id: ID);
    if(result){
      await fetchpayroll();
    }
